    ...
    if args.task != "tune":
        ...
        for row in df.iterrows():
            for oracle_name in args.oracles:
                ...
                res = row[1]['Model Response']
                assert args.task == 'simple'
                ...
                buffer = optimizer.optimize(oracle=oracle, config=config_default, seed=args.seed[0])
                if buffer == None:
                    top_ks.append([None] * num_k)
                    continue
                # for mol, info in list(buffer.items())[:5]:
                #     score = info[0]
                #     n_runs = info[1] # not actually sure what second element is
                #     print(mol, score)
                top_ks.append([item[0] for item in list(buffer.items())[:num_k]])
                # print(len(buffer))
        ...
        for i in range(num_k):
            df[f'top_{i}'] = [el[i] for el in top_ks]
        print('Saving csv to:', os.path.join(args.output_dir, "out_" + args.csv.split('/')[-1]))
        df.to_csv(os.path.join(args.output_dir, "out_" + args.csv.split('/')[-1]), index=False)
